-- Planning Poker: схема Supabase
-- Выполните целиком в Supabase Dashboard → SQL Editor.
...
as $$
  select exists (
    ...
    where tm.team_id = p_team_id
      and tm.active = true
      and lower(tm.email) = public.current_user_email()
  );
$$;
...
create or replace function public.is_issue_lead(p_issue_id uuid)
returns boolean
language sql
stable
security definer
set search_path = public
as $$
  select public.is_team_lead(public.issue_team_id(p_issue_id));
$$;
...
set search_path = public
as $$
  select public.can_access_issue(p_issue_id)
    and exists (
      select 1 from public.issues i
      where i.id = p_issue_id
        and i.status = 'voting'
...
begin
  ...
  select
    (select count(*) from public.votes v
      where v.issue_id = p_issue_id and v.round = v_round),
    (select count(*) from public.team_members tm
      where tm.team_id = v_team and tm.active = true);
end;
$$;
...
for insert to authenticated
with check (public.is_team_lead(team_id) and created_by = auth.uid());
...
with check (
  ...
  and exists (
    select 1 from public.sessions s
    where s.id = session_id and public.is_team_lead(s.team_id)
  )
);
...
using (
  user_id = auth.uid()
  or (public.can_access_issue(issue_id) and public.issue_is_revealed(issue_id))
);
...
with check (
  user_id = auth.uid()
  and lower(voter_email) = public.current_user_email()
...
grant execute on function public.is_issue_lead(uuid) to authenticated;
...
-- Защита security definer функций от публичного вызова без авторизации.
...
revoke all on function public.is_issue_lead(uuid) from public, anon;
